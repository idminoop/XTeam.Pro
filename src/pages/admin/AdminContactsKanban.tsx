...
export default function AdminContactsKanban() {
  ...
  if (loading) {
    return <div className="flex h-56 items-center justify-center text-sm text-gray-400">Loading...</div>;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Contacts Kanban</h1>
          <p className="text-sm text-gray-500">Drag cards between stages to update pipeline.</p>
        </div>
        <button
          onClick={load}
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-5">
        {(Object.keys(stageMap) as PipelineStage[]).map(stage => (
          ...
          >
            ...
            <div className="space-y-2 p-3 min-h-[320px]">
              {stageMap[stage].map(card => (
                ...
                >
                  ...
                  {savingId === card.id && <p className="mt-2 text-[10px] text-gray-400">Saving...</p>}
                </div>
              ))}
            </div>
          </div>
